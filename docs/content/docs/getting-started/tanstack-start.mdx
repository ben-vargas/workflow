...
<Callout>
While Tanstack Start provides the build tooling and development server, Nitro creates the workflow routes and adds client transforms needed for Workflow DevKit. Learn more about Nitro [here](https://v3.nitro.build).
</Callout>

### Configure Tanstack Start

Add `nitro()` and `workflow()` to your Vite config. This enables usage of the `"use workflow"` and `"use step"` directives.
