...
After [building a durable AI agent](/guides/ai), we already get UI message chunks for displaying tool invocations and return values. However, for long-running steps, we may want to show progress updates, or stream step output to the user while it's being generated.
  ...
  if (part.type === 'data-found-flight') { // [!code highlight]
    ...
    return ( // [!code highlight]
      <div key={`${part.id}-${flight.flightNumber}`} className="p-3 bg-muted rounded-md"> // [!code highlight]
        ...
        <div className="font-medium">${flight.price}</div> // [!code highlight]
      </div> // [!code highlight]
    ); // [!code highlight]
  } // [!code highlight]
...
- [Building Durable AI Agents](/guides/ai) - Complete guide to durable agents
