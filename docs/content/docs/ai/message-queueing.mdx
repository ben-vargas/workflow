...
<Callout type="info">
If you just need basic multi-turn conversations where messages arrive between turns, see [Chat Session Modeling](/guides/ai/chat-session-modeling). This guide covers the more advanced case of injecting messages *during* turns.
</Callout>
...
prepareStep: ({ stepNumber, messages }) => {
  // Use a stronger model for complex multi-step operations
  if (stepNumber > 2 && messages.length > 10) {
    return { model: 'bedrock/claude-4-sonnet-20250514-v1' };
  }
  return {};
}
```
...
- [Chat Session Modeling](/guides/ai/chat-session-modeling) - Single-turn vs multi-turn patterns
- [Building Durable AI Agents](/guides/ai) - Complete guide to creating durable agents
