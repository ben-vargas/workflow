...
As discussed in [Streaming Updates from Tools](/guides/ai/streaming-updates-from-tools), it's common to use a step just to call `getWritable()` for writing custom data parts to the stream.
