...
Let's add stream resumption to our Flight Booking Agent that we build in the [Building Durable AI Agents](/guides/ai) guide.
...
The `startIndex` parameter ensures the client can choose where to resume the stream from.
